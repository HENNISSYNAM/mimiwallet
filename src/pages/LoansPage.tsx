...
const LOAN_TYPE_KEYS = ['workingCapital', 'invoiceAdvance', 'expansion'] as const;
...
const statusStyle: Record<string, { cls: string; dot: string }> = {
  on_track: { cls: 'text-mimi-green', dot: 'bg-mimi-green' },
  completed: { cls: 'text-mimi-green', dot: 'bg-mimi-green' },
  due_soon: { cls: 'text-mimi-amber', dot: 'bg-mimi-amber' },
  pending: { cls: 'text-muted-foreground', dot: 'bg-muted-foreground' },
};

const scoreFactorKeys = [
  { key: 'paymentHistory', score: 89 },
  { key: 'cashFlowHealth', score: 76 },
  { key: 'capitalUsageRatio', score: 91 },
  { key: 'operatingTime', score: 78 },
];
...
export default function LoansPage() {
  ...
  const handleApply = async () => {
    ...
    const { error } = await supabase.from('loan_applications').insert({
      ...
      amount: loanAmount,
      term_days: loanTerm,
      loan_type: t(`fin.loans.loanTypes.${LOAN_TYPE_KEYS[loanType]}`),
      status: 'pending',
      due_date: new Date(Date.now() + loanTerm * 86400000).toISOString().slice(0, 10),
    });
    setSubmitting(false);
    if (error) {
      toast.error(t('fin.loans.toast.applyFailed', { error: error.message }));
      return;
    }
    toast.success(t('fin.loans.toast.applySuccess'));
    loadLoans();
  };
  ...
  return (
    <motion.div variants={stagger} initial="hidden" animate="show" className="space-y-6">
        ...
        <motion.div variants={fadeUp} className="bg-card/60 backdrop-blur-sm border border-border/60 rounded-2xl p-8 flex flex-col items-center justify-center relative overflow-hidden">
          ...
          <div className="relative z-10 text-center">
            <svg viewBox="0 0 200 130" className="w-52 mb-6">
              ...
              <text x="100" y="110" textAnchor="middle" fill="hsl(var(--text-secondary))" fontFamily="Inter, -apple-system, sans-serif" fontSize="12">{t('fin.loans.creditScore.rank')}</text>
            </svg>
            <p className="text-xs text-muted-foreground">{t('fin.loans.creditScore.updated')}</p>
            <p className="text-xs text-muted-foreground mt-1">{t('fin.loans.creditScore.higherThan')}</p>
          </div>
        </motion.div>

        <motion.div variants={fadeUp} className="bg-card/60 backdrop-blur-sm border border-border/60 rounded-2xl p-6">
          <h3 className="font-display font-bold text-foreground text-lg mb-6">{t('fin.loans.creditScore.analysisTitle')}</h3>
          <div className="space-y-5">
            {scoreFactorKeys.map((f) => (
              <div key={f.key}>
                <div className="flex justify-between text-sm mb-2">
                  <span className="text-muted-foreground font-medium">{t(`fin.loans.creditScore.factors.${f.key}`)}</span>
      ...
      {/* Loan Calculator */}
      <motion.div variants={fadeUp} className="bg-card/60 backdrop-blur-sm border border-border/60 rounded-2xl p-6">
        <h3 className="font-display font-bold text-foreground text-lg mb-6">{t('fin.loans.calculator.title')}</h3>
        <div className="flex gap-1 bg-accent/30 rounded-xl p-1 mb-6 w-fit">
          {LOAN_TYPE_KEYS.map((key, i) => (
            <button
              key={key}
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          <div className="space-y-8">
            <div>
              <div className="flex items-baseline justify-between mb-3">
                <label className="text-sm text-muted-foreground font-medium">{t('fin.loans.calculator.loanAmount')}</label>
            ...
            <div>
              <label className="text-sm text-muted-foreground font-medium mb-3 block">{t('fin.loans.calculator.term')}</label>
              <div className="flex gap-2">
                {[30, 60, 90, 180].map((d) => (
                  ...
                  >
                    {d}{t('fin.loans.calculator.termUnit')}
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="bg-gradient-to-br from-primary/5 to-mimi-green/5 border border-primary/10 rounded-2xl p-6 flex flex-col justify-between">
            <div className="space-y-4">
              <div>
                <p className="text-sm text-muted-foreground mb-1">{t('fin.loans.calculator.receivedAmount')}</p>
                <p className="font-mono text-3xl font-bold text-mimi-green">{formatVND(received)}</p>
              </div>
              <div className="border-t border-border/30 pt-4 space-y-3">
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">{t('fin.loans.calculator.fee')}</span>
                  <span className="font-mono text-foreground">{formatVND(fee)} <span className="text-muted-foreground">{t('fin.loans.calculator.feeRate')}</span></span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">{t('fin.loans.calculator.dueDate')}</span>
                  <span className="text-foreground">{formatDateShort(new Date(Date.now() + loanTerm * 86400000))}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">{t('fin.loans.calculator.effectiveRate')}</span>
                  <span className="font-mono text-foreground">{((fee / loanAmount) * (365 / loanTerm) * 100).toFixed(1)}{t('fin.loans.calculator.perYear')}</span>
                </div>
              </div>
            </div>
            <motion.button
            ...
            >
              {submitting && <Loader2 size={14} className="animate-spin" />} {t('fin.loans.calculator.applyNow')}
            </motion.button>
          </div>
        </div>
      </motion.div>
      ...
      <motion.div variants={fadeUp} className="bg-card/60 backdrop-blur-sm border border-border/60 rounded-2xl overflow-hidden">
        <div className="p-6 border-b border-border/30">
          <h3 className="font-display font-bold text-foreground text-lg">{t('fin.loans.activeLoans.title', { count: loanList.length })}</h3>
        </div>
        {loading ? (
          ...
          <div className="text-center py-16 px-6">
            <p className="text-sm text-foreground font-medium">{t('fin.loans.activeLoans.emptyTitle')}</p>
            <p className="text-xs text-muted-foreground mt-1">{t('fin.loans.activeLoans.emptyDesc')}</p>
          </div>
        ) : (
            ...
            <tbody>
              {loanList.map((l, i) => {
                ...
                >
                  ...
                  <td className="px-5 py-4">
                    <span className={`inline-flex items-center gap-1.5 text-xs font-medium ${statusStyle[status].cls}`}>
                      <span className={`w-1.5 h-1.5 rounded-full ${statusStyle[status].dot}`} />
                      {t(`fin.loans.status.${status}`)}
                    </span>
                  </td>
                </motion.tr>
                );
