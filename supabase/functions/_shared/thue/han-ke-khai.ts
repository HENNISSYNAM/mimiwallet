 ...
 *
 * Chuyển từ `src/lib/hanKeKhai.ts` về đây ngày 15/09/2026: MIMI Assistant phải hỏi đúng
 * kỳ mà màn Chứng từ chi phí đang hiện, nếu không hai nơi báo hai con số "chưa có giấy
 * tờ". `src/lib/hanKeKhai.ts` giờ chỉ xuất lại file này. File không import gì.
 *
 * MÚI GIỜ. Hàm dùng giờ địa phương của nơi chạy. Trình duyệt ở Việt Nam là UTC+7; edge
 * function chạy UTC nên người gọi ở máy chủ phải truyền `luc` đã dời sang giờ Việt Nam
 * (xem `lucGioVietNam` bên dưới).
 *
...
/**
 * Khoảng ngày của quý đang tới hạn — đúng khoảng màn Chứng từ chi phí đọc: ba tháng kết
 * thúc trước tháng của ngày hạn. Trả YYYY-MM-DD.
 */
export function khoangNgayKyKeKhai(ky: KyKeKhai): { tu: string; den: string } {
  const cuoiKy = new Date(ky.han.getFullYear(), ky.han.getMonth(), 0);
  const dauKy = new Date(cuoiKy.getFullYear(), cuoiKy.getMonth() - 2, 1);
  return { tu: iso(dauKy), den: iso(cuoiKy) };
}

/**
 * "Bây giờ" ở Việt Nam, biểu diễn bằng một Date mà các getter giờ địa phương đọc ra giờ
 * Việt Nam khi chạy ở máy UTC (edge function). Không dùng ở trình duyệt.
 */
export function lucGioVietNam(luc: Date = new Date()): Date {
  const vn = new Date(luc.getTime() + 7 * 3_600_000);
  return new Date(vn.getUTCFullYear(), vn.getUTCMonth(), vn.getUTCDate(), vn.getUTCHours(), vn.getUTCMinutes(), vn.getUTCSeconds());
}
